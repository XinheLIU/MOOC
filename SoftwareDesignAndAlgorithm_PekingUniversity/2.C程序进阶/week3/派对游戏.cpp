...
输入用一个字符串代表小朋友队列。字符串中只会出现两个字符（样例输入里用的是 括号但实际数据则不一定），分别代表小男孩和小女孩，首先出现的字符代表小男孩，另一个字符代表小女孩。小孩总数不超过100
...
int match(int n){
    ...
    if (input[n] == girl)        //开头没有boy，结束程序（不能配对）
        return n;
    if (input[n] == 0)
        return n;
    ...
    while(input[r] != 0 && input[r] != girl){
        ...
        r = match(r);                      //递归, 能配对的领走, return n+2 (r+1) (移动两位），再次调用
    }
    
    if(input[r] == girl){
        cout << n << ' ' << r << endl;    //配对成功 r+1
        return r+1;
